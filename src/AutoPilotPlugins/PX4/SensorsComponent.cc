    ...
    _name(tr("Sensors"))
{
    ...
    if (_vehicle->fixedWing() || _vehicle->vtol() || _vehicle->airship()) {
        ...
        if (_vehicle->firmwareMajorVersion() >= 1 && _vehicle->firmwareMinorVersion() >= 14) {
            _airspeedCalTriggerParams << "SYS_HAS_NUM_ASPD" << "FW_USE_AIRSPD";
        } else {
...
bool SensorsComponent::setupComplete(void) const
{
    ...
    if (_vehicle->fixedWing() || _vehicle->vtol() || _vehicle->airship()) {
        if (_vehicle->firmwareMajorVersion() >= 1 && _vehicle->firmwareMinorVersion() >= 14) {
            if (_vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId, "FW_USE_AIRSPD")->rawValue().toBool() &&
                    _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId, "SYS_HAS_NUM_ASPD")->rawValue().toBool() &&
